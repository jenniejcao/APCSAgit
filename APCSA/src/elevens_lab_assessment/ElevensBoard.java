package elevens_lab_assessment;
...
public class ElevensBoard extends Board{
	...
	public boolean isLegal(List<Integer> selectedCards) {
		...
		else if (selectedCards.size()==3&&contains3Sum11(selectedCards)){
				return true;
		}
		return false;
	}
	...
	public boolean anotherPlayIsPossible() {
		...
		//	cardBoard.add(i);
		//}
		if (containsPairFace(cardIndexes())||contains3Sum11(cardIndexes())){
			return true;
		}
		else {
	...
	private boolean containsPairFace(List <Integer> selectedCards){
		for (int first:selectedCards){
			for (int second:selectedCards){
				if (first!=second && (cards[first].pointValue()==0 && cards[second].pointValue()==0)){
					if (cards[first].rank().equals(cards[second].rank())){
						return true;
					}
				}
			}
